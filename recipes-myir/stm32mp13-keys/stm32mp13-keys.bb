...
STM32MP13_KEY_DIR ??= "${DEPLOY_DIR_IMAGE}/stm32mp13-key"
...
do_keygen() {
    ...
    mkdir -p "${STM32MP13_KEY_DIR}"
    
     # Check if forced regeneration or keys don't exist
    local need_generate=0
    if [ "${STM32MP13_FORCE_KEYGEN}" = "1" ]; then
        bbnote "Force regenerating STM32MP13 keys as requested"
        need_generate=1
    elif [ ! -f "${STM32MP13_KEY_DIR}/privateKey00.pem" ] || \
         [ ! -f "${STM32MP13_KEY_DIR}/stm32mp_encryption_key.bin" ] || \
         [ ! -f "${STM32MP13_KEY_DIR}/stm32mp_encryption_key_256bits.bin" ]; then
        bbnote "Some keys are missing, generating new keys"
        need_generate=1
    else
        bbnote "STM32MP13 keys already exist, skipping generation"
        need_generate=0
    fi
    
    if [ ${need_generate} -eq 1 ]; then
        bbnote "Generating STM32MP13 keys in ${STM32MP13_KEY_DIR}"
        
         # Generate key pairs
        bbnote "Generating 8 RSA key pairs..."
        STM32_KeyGen_CLI -abs "${STM32MP13_KEY_DIR}/" \
            -pwd 0000 1111 2222 3333 4444 5555 6666 7777 \
            -n 8
        
        if [ $? -ne 0 ]; then
            bberror "Failed to generate STM32MP13 key pairs"
            exit 1
        fi
        
        # Generate FSBL encryption key
        bbnote "Generating FSBL encryption key (16 bytes)..."
        STM32_KeyGen_CLI -rand 16 "${STM32MP13_KEY_DIR}/stm32mp13_encryption_key.bin"
	...
	cat "${STM32MP13_KEY_DIR}/stm32mp13_encryption_key.bin" \
  	   "${STM32MP13_KEY_DIR}/stm32mp13_encryption_key.bin" \
    	> "${STM32MP13_KEY_DIR}/stm32mp13_encryption_key_256bits.bin"
        #STM32_KeyGen_CLI -rand 32 "${STM32MP13_KEY_DIR}/stm32mp13_encryption_key_256bits.bin" 
        if [ $? -ne 0 ]; then
            bberror "Failed to generate FIP encryption key"
            exit 1
        fi
	bbnote "Note: 256-bit key is just 128-bit key duplicated for STM32MP13x compatibility"
        # Set secure permissions
        chmod 600 "${STM32MP13_KEY_DIR}"/*.pem 2>/dev/null || true
        chmod 600 "${STM32MP13_KEY_DIR}"/*.bin 2>/dev/null || true
        
        # Generate key fingerprint file
        bbnote "Generating key fingerprints..."
        {
            echo "STM32MP13 Key Fingerprints"
            echo "Generated: $(date)"
            echo "Key Directory: ${STM32MP25_KEY_DIR}"
            echo ""
            echo "Key Files:"
            ls -la "${STM32MP13_KEY_DIR}/"
        } > "${STM32MP13_KEY_DIR}/key_generation_info.txt"
        
        chmod 644 "${STM32MP13_KEY_DIR}/key_generation_info.txt"
        
        bbnote "STM32MP13 keys generated successfully in ${STM32MP13_KEY_DIR}"
    fi
}
