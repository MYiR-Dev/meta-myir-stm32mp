...
burn_rootfs_a()
{
	...
	cmd_check $? "Update rootfs_a"
	sync
	mount ${DRIVE}p8 /mnt
	cp ${imagedir}/Image /mnt/boot 
	cp ${imagedir}/*.dtb /mnt/boot 
	umount /mnt
}

burn_rootfs_b()
{
        dd if=${ROOTFS_FILE} of=${DRIVE}p9 bs=1M conv=fsync
        cmd_check $? "Update rootfs_b"
        sync
	mount ${DRIVE}p9 /mnt
        cp ${imagedir}/Image /mnt/boot
        cp ${imagedir}/*.dtb /mnt/boot
...
burn_user_rootfs()
{
	dd if=${USERFS_FILE} of=${DRIVE}p10 bs=1M conv=fsync
...
EOF
	...
	e2fsck -f  ${DRIVE}p10 <<EOF
        y
EOF
        resize2fs   ${DRIVE}p10 <<EOF
        y
EOF
...
echo "---------------------------start rootfs_a ---------------------------" > /dev/ttySTM0
burn_rootfs_a
echo "---------------------------end rootfs_a---------------------------" > /dev/ttySTM0
echo "\n"


echo "---------------------------start rootfs_b ---------------------------" > /dev/ttySTM0
burn_rootfs_b
echo "---------------------------end rootfs_b---------------------------" > /dev/ttySTM0
echo "\n"
